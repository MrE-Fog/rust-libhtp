...
fn main() {
    ...
    let out_dir = env::var_os("OUT_DIR")
        .map(|s| PathBuf::from(s).join("libhtp"))
        .unwrap();
    ...
    // The directory where the includes installed by libhtp can be found.
    ...
    fs_extra::dir::copy(manifest_dir.join("libhtp"), &build_dir, &opts).unwrap();
    ...
    println!("cargo:rustc-link-lib=static=htp");
